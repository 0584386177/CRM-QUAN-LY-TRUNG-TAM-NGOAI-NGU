...
<div class="card card-primary card-outline mb-4 " bis_skin_checked="1"> <!--begin::Header-->
    ...
    <form method="POST" action="{{route('subject.update', $subject->id)}}"> @csrf <!--begin::Body-->
        <div class="card-body">
            <div class="row"> <!-- Cột trái -->
                <div class="col-md-6">
                    <div class="mb-3"> <label for="fullname" class="form-label">Tên môn học</label> <input type="text"
                            name="name" value="{{$subject->name}}" class="form-control"> </div>
                    <div class="mb-3"> <label class="form-label">Học phí</label> <input type="text" name="fee"
                            value={{@price($subject->fee ?? 0) }} class="form-control">
                    </div>
                </div> <!-- Cột phải -->
                <div class="col-md-6">
                    <div class="mb-3"> <label class="form-label">Phụ trách môn </label>
                        <select name="teacher_id" class="form-select">
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ $subject->teachers->contains('id', $teacher->id) ? 'selected' : '' }}>
                                {{ $teacher->fullname }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3"> <label class="form-label">Số buổi học</label> <input type="text"
                            name="number_of_lessions" value="{{$subject->number_of_lessions}}" class="form-control">
                    </div>
